...
#     co2_emissions = st.number_input("CO2 Emissions (in kg)", min_value=0.0, value=2000.0)
#     fuel_consumption = st.number_input("Fuel Consumption (in litres)", min_value=0.0, value=1000.0)
#     st.caption("📌 CO2 in kg | Fuel in litres | Distance in km | Efficiencies between 0.0 and 1.0")
#     if fuel_consumption > 0:
#         emission_efficiency = co2_emissions / fuel_consumption
#         st.metric(label="Emission Efficiency", value=f"{emission_efficiency:.4f}")
#     else:
#         st.warning("⚠️ Please enter valid fuel consumption to compute emission efficiency.")
...
#             pdf.cell(0, 8, f"CO2 Emissions (Input): {co2_emissions} kg", ln=True)
#             pdf.cell(0, 8, f"Fuel Consumption: {fuel_consumption} litres", ln=True)
#             pdf.cell(0, 8, f"Calculated Emission Efficiency: {emission_efficiency:.4f}", ln=True)
...
#         st.write(f"**Distance fixed at:** `{distance}` km")
#         comp_results = []
